...
proc root_handler {args} {
    set cookie_value [ns_getcookie third_visit 0]
...
ns_register_proc GET "/images/*.jpeg" {
    set path [ns_server pagedir]
    set requested_file [ns_conn url]
    # Extract just the filename from the URL
    set filename [file tail $requested_file]
    # Build the full path, ensuring we only serve jpeg files
    set filepath "${path}/assets/$filename"
    
    if {[file exists $filepath] && [file extension $filepath] eq ".jpeg"} {
        ns_returnfile 200 image/jpeg $filepath
    } else {
        ns_return 404 text/plain "Image not found"
    }
}
